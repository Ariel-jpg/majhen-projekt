...
class CitizenState:
    ...
    def recieve_friend_request(self, sender, friend_request) -> None: # "Listener" - c2 Method
        ...
        if response == "y":
            self.state.friend_list.update({ sender.get_cuil() : sender.get_formatted_data() })
    ...
    def recieve_answer_friend_request(self, friend_request) -> None: # "Listener" - c1 Method
        ...
        if accepted:
            ...
            self.state.friend_list.update({ friend.get_cuil() : friend.get_formatted_data() })
        else:
            ...
            if bool(self.rejections_list.get(citizen_id)):
                ...
                if rejections_counter == 5:
                    ...
                    print(f"Usted fue bloqueado para el usuario {citizen.name} {citizen.last_name}. Motivo: 5 o más solicitudes de amistad rechazadas") # TODO
            else:
                rejections_counter = 1
...
class Citizen:
    ...
    def exists_citizen_blocked(self, citizen_cuil) -> bool:
        return citizen_cuil in self.state.block_list
    ...
    def notify_event_with_friend(self, type_event, friend):
        if self.state.friend_list.get(friend.cuil):
    ...
    def recieve_friend_request(self, sender, friend_request) -> None:
        self.state.recieve_answer_friend_request(friend_request) # "Listener" - c2 Method
    def recieve_answer_friend_request(self, friend_request) -> None:
        self.state.recieve_answer_friend_request(friend_request)# "Listener" - c1 Method